...
def batch_gpt_task(sample, max_tokens, tokenizer):
    ...
    sample_input_ids = torch.LongTensor(
        tokenizer.encode(sample, max_length=max_tokens, truncation=True)
    )
    ...
    target_ids = (
        sample_input_ids.detach().clone()
    )  # Will be shifted right inside the model.
    target_ids[target_ids == tokenizer.eos_token_id] = -100

    # import pdb; pdb.set_trace()

    return {"input_ids": sample_input_ids, "labels": target_ids}
...
def batch_bart_task(sample, max_tokens, tokenizer, mask_probability):
    ...
    mask = torch.bernoulli(
        torch.ones_like(sample_input_ids) * mask_probability
    )  # 1's in mask_probability% of the places

    target_ids = sample_input_ids.detach().clone()
    target_ids[target_ids == tokenizer.pad_token_id] = -100
    target_ids[mask == 0] = -100

    sample_input_ids = (sample_input_ids * (1 - mask)) + (
        torch.ones_like(sample_input_ids) * tokenizer.mask_token_id * mask
    )  # Mask input
...
def dummy_gpt_task(max_tokens):
    ...
    return {"input_ids": seq, "labels": seq, "attention_mask": torch.ones_like(seq)}
...
def _T5_mask(sample, mask_probability, tokenizer):
    ...
    mask = torch.bernoulli(
        torch.ones_like(sample) * mask_probability
    ).bool()  # 15 % are 1s
...
def _T5_apply_mask(sample, mask, tokenizer, hide_sentinels=False):
    ...
    sample_not_padding_tokens = torch.logical_not(
        torch.eq(sample, tokenizer.pad_token_id)
    )
...
def apply_mask_denoising(sample, max_tokens, tokenizer, mask_probability):
    ...
    sample_input_ids = torch.LongTensor(
        tokenizer.encode(
            sample, padding="max_length", max_length=max_tokens, truncation=True
        )
    )

    masked_sample_input_ids, masked_sample_labels = _T5_mask(
        sample_input_ids, mask_probability, tokenizer
    )
    ...
    if N_pad_labels > 0:
        masked_sample_labels = F.pad(
            masked_sample_labels,
            [0, N_pad_labels],
            mode="constant",
            value=tokenizer.pad_token_id,
        )
...
def _BERT_mlm_mask(sample, mask_probability, tokenizer):
    ...
    new_sample = torch.where(mask, sentinel_idxs, sample)
...
def apply_mask_bert_mlm(sample, max_tokens, tokenizer, mask_probability):
    ...
    masked_sample_input_ids, masked_sample_labels = _BERT_mlm_mask(
        sample_input_ids, mask_probability, tokenizer
    )
